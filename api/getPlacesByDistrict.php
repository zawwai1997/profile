...
$data = $user->getAll('District.name');
...
foreach ($data as $key => $value) {
    ...
    $total_suspected = $total_lab_negative + $value['suspected'];
    ...
    $OneHospitalAry = array(
        ...
        "total_cases" => $value['pui'] + $value['suspected'] + $value['lab_negative'] + $value['lab_pending'] +
            $value['death'] + $value['lab_confirmed'] + $value['recovered']+$total_confirmed
    );

    if(!isset($districtAry[$value['district']])) {
        $districtAry[$value['district']] = array();
    }

    $districtAry[$value['district']][] = $OneHospitalAry;
}
$overall = array(
    "total_pui" => $total_pui,
    ...
    "total_cases" => $total_pui+$total_suspected+$total_lab_negative+$total_lab_pending+$total_lab_death+$total_recovered
);


$finalAry = array("overall" => $overall, "details" => $districtAry);
